...
`dashboard` dokłada do `analytics` warstwę porównawczą. `previous_period_rows` pobiera okres tuż przed wybranym, tej samej długości, i działa tylko przy podanych `start` i `end` — bez nich `comparison.previous_rate` jest `None`, a interfejs pisze o braku okresu porównawczego. `momentum` liczy różnicę średnich z ostatnich 14 i wcześniejszych 14 punktów trendu, więc wymaga co najmniej 28 zamkniętych dni. `most_improved` i `most_regressed` biorą tylko nawyki obecne w obu okresach; flaga `reliable` oznacza minimum 5 rekordów po obu stronach. `habit_behavior` mierzy przerwy i mediany powrotów na rekordach zakończonych, a `goal_metrics` zapas względem celu i rekord osobisty — dla `Breaking` mniej znaczy lepiej, a przy `goal == 0` zamiast średniego stosunku raportowane są okresy bez naruszenia. Sufit 999% w `goal_metrics` chroni przed dzieleniem przez ilości bliskie zeru.

Świadomie nie przeniesiono tu dwóch sekcji z Habit Lens: współwystępowania nawyków (współczynnik phi na danych jednej osoby jest zbyt słabą przesłanką) oraz jakości danych (tam luka w pokryciu sygnalizuje problem z synchronizacją, a przy imporcie snapshotem CSV oznacza po prostu, że nawyku wtedy nie było).
